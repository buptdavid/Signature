...
function deleteScene(id){
	if(confirm('您确认取消推荐吗？')){
		 window.location.href="delete_scene.html?originalId="+id; 
	}else{
		return;
	}
}
  ...
  <table width="100%" border="0" cellpadding="0" cellspacing="1" class="listtable" >
    ...
    <s:iterator value="originals" id="original">
       <tr>
        ...
        <td align="left"><s:property value="#original.parentName"/></td>
        <td align="center"><s:property value="#original.className"/></td>
        <td align="center"><s:date name="#original.time" format="yyyy-MM-dd HH:mm"/></td>
        <td class="list_aselect"><a href="javascript:deleteScene(<s:property value='#original.id'/>);"  >取消推荐</a></td>
        </tr>
     </s:iterator>
    </tbody>
    <tfoot>
